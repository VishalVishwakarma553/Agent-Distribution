...
export const signUp = async(req, res) => {
    try{
        ...
        const token = await jwt.sign({userId: newUser._id}, process.env.SECRET_KEY, {expiresIn: "2d"})
...
export const login = async(req, res) => {
    try{
        ...
        //Generate token
        const token = jwt.sign({userId:userExistence._id}, process.env.SECRET_KEY, {expiresIn: "2d"})
        //Send response
        return res.status(200).cookie("token", token, {maxAge: 1*24*60*60*1000}).json({
